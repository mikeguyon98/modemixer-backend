...
class ItemGenerator:
    ...
    def get_context(query: str, gender: str, k: int = 2) -> tuple[str, list[str]]:
        ...
        vector_search_woman = MongoVectorStore(
            atlas_connection_string, "langchain_db", "FemaleTrend", vector_search_index
        ).initialize()
        vector_search_men = MongoVectorStore(
            atlas_connection_string, "langchain_db", "MaleTrend", vector_search_index
        ).initialize()
    ...
    @staticmethod
    def item_description_chain(
        item_name: str, gender: str, collection_description: str = ""
    ) -> str:
        ...
        system_message = {
            "role": "system",
            "content": (
                "You are a fashion designer and you are assigned with the task of designing a new "
        ...
        user_message = {"role": "user", "content": f"ITEM NAME: {item_name}"}
